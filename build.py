...
def generate_site(target_dir: Path, asset_prefix: str = '', is_new_v1: bool = False):
    ...
    (target_dir / 'media.html').write_text(
        ...
        f'''<main id="main"><section class="media-intro"><p class="eyebrow">สื่อประชาสัมพันธ์ · 2570</p><h1>โปสเตอร์และ<br><span>อินโฟกราฟิกทั้งหมด</span></h1><p>คลิกภาพเพื่อเปิดดูแบบสไลด์และดาวน์โหลดไฟล์</p></section><section class="media-grid" aria-label="โปสเตอร์และอินโฟกราฟิก">{gallery_items}</section></main><dialog class="media-modal" aria-label="แสดงภาพสื่อประชาสัมพันธ์"><button class="modal-close" type="button" aria-label="ปิด">×</button><a class="modal-download" href="#" download aria-label="ดาวน์โหลดภาพ">⇩</a><div class="modal-view"><button type="button" data-direction="previous" aria-label="ภาพก่อนหน้า">‹</button><img class="modal-image" src="" alt=""><button type="button" data-direction="next" aria-label="ภาพถัดไป">›</button></div></dialog>{modal_script}''' +
        get_footer(is_new_v1=is_new_v1),
        encoding='utf-8'
    )
